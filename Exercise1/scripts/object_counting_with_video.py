"""
Object Counting and Video Annotation Script Using YOLO and Region Counter

This script processes a video file to count vehicles within a specified region of interest (ROI) using a YOLO model.
It reads ROI coordinates from a JSON file, processes the video at a target frame rate, writes the vehicle count
per frame to a CSV file, and saves an annotated output video. A progress bar is displayed during processing.

Modules:
    - csv: For writing vehicle counts to a CSV file.
    - json: For loading ROI coordinates from a JSON file.
    - cv2: OpenCV library for video capture, processing, and writing.
...
Configuration:
    ...
    - OUTPUT_VIDEO_PATH: Path to the output annotated video file.
...
Usage:
    ...
    generate an annotated video, and create a CSV file with vehicle counts per frame.
"""
...
OUTPUT_VIDEO_PATH = '../outputs/region_counting.avi'  # Path to the output annotated video file
OUTPUT_CSV_PATH = '../outputs/counts_per_frame_video_1.csv'  # Path to the output CSV file
...
video_writer = cv2.VideoWriter(OUTPUT_VIDEO_PATH, cv2.VideoWriter_fourcc(*"mp4v"), TARGET_FPS,
                               (w, h))  # Initialize video writer for annotated output
...
with tqdm(total=total_frames, desc="Processing Video Frames") as pbar:  # Initialize progress bar
    ...
    while cap.isOpened():
        ...
        if frame_count % frame_skip == 0:
            ...
            results = regioncounter(im0)  # Get region counts and annotated frame
            video_writer.write(results.plot_im)  # Write annotated frame to output video
...
# --- Cleanup ---
print(f"\nProcessing complete. Output video saved to {OUTPUT_VIDEO_PATH}")  # Notify user of completion
print(f"Frame-by-frame counts saved to {OUTPUT_CSV_PATH}")  # Notify user of CSV output
